...
class targetBall {
    ...
    drawballStatic() {
        context.beginPath();
        context.arc(this.x, this.y, this.r - 70, 0, 2 * Math.PI, false);
...
class ball {
    ...
    intersects(other) {
        ...
        if (m_Distance < other.r && Math.sqrt(Math.pow(localX - this.vHeadingX * 100, 2) + Math.pow(localY - this.vHeadingY * 100, 2)) < other.r || Math.sqrt(Math.pow(localX, 2) + Math.pow(localY, 2)) < other.r) {
            drawLine(other.x, other.y, this.x + this.vHeadingX * 50, this.y + this.vHeadingY * 50);            
            drawArrow(this.x, this.y, this.x + P.x * 50, this.y + P.y * 50,'blue');
            return true;
        }
        else
        return false;
    ...
    drawvHeading() {
        drawArrow(this.x, this.y, this.x + this.vHeadingX * 50, this.y + this.vHeadingY * 50,"red");
    }
...
function iterate() {
    ...
    for (var i = 0; i < balls.length; i++) {
        ...
        if (balls[i].intersects(a)) {
            nextX = balls[i].x + (balls[i].vHeadingX) * velocity  + P.x * velocity *0.8;//+ F.x * velocity * 0.4;
            nextY = balls[i].y + (balls[i].vHeadingY) * velocity  + P.y * velocity *0.8;//+ F.y * velocity * 0.4;
