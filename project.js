...
class ball {
    constructor(x, y, r = 0, velocityX = 0, velocityY = 0, length = 50) {
        this.x = x;
        this.y = y;
        this.r = r;
    ...
    intersects(other) {
        ...
        let c = -this.vHeadingX * this.uX - this.vHeadingY * this.uY;
        let localX = other.x - this.x;
        let localY = other.y - this.y;
        let m_Distance = Math.abs(this.uX * localX + this.uY * localY + c) / Math.sqrt(Math.pow(this.uX, 2) + Math.pow(this.uY, 2));
        // console.log("Distance =",m_Distance);
        let P = new targetBall(0,0,0);
        // find normal vector
        P.x = (localX / Math.sqrt(Math.pow(localX, 2) + Math.pow(localY, 2)) - this.vHeadingX) * -1 ;
        P.y = (localY / Math.sqrt(Math.pow(localX, 2) + Math.pow(localY, 2)) - this.vHeadingY) * -1;

        P.x = P.x / Math.sqrt(Math.pow(P.x, 2) + Math.pow(P.y, 2));
        P.y = P.y / Math.sqrt(Math.pow(P.x, 2) + Math.pow(P.y, 2));
        // console.log("P = ", P.x, " ; ", P.y);

        if (m_Distance < other.r && Math.sqrt(Math.pow(localX - this.vHeadingX * 100, 2) + Math.pow(localY - this.vHeadingY * 100, 2)) < other.r || Math.sqrt(Math.pow(localX, 2) + Math.pow(localY, 2)) < other.r) {
            //this.drawLine(other.x, other.y, this.x + this.vHeadingX * 50, this.y + this.vHeadingY * 50);
            //drawArrow(this.x, this.y, this.x + P.x * 50, this.y + P.y * 50);
            return {
                bool : true,
                x: P.x,
                y: P.y
...
function init() {
    for (let i = 0; i < 30; i++){
        let w = new targetBall(Math.floor(Math.random() * 700), Math.floor(Math.random() * 700), 80);
        avoidObject.push(w);
    }
    boolInit = false;
}
...
function iterate() {
    ...
    if (mouseIsDown) {
        ...
        c = new ball(x, y, r, firstX, firstY, 60);
        balls.push(c);
    ...
    for (var i = 0; i < balls.length; i++) {
        ...
        P.x = 0;
        P.y = 0;
        avoidObject.forEach(element => {
            let t = balls[i].intersects(element);
            if (t.bool){
        ...
        if (c) {
            nextX = balls[i].x + (balls[i].vHeadingX) * velocity  + P.x * velocity *0.5;//+ F.x * velocity * 0.4;
            nextY = balls[i].y + (balls[i].vHeadingY) * velocity  + P.y * velocity *0.5;//+ F.y * velocity * 0.4;
